...
export default function AppCommandPage() {
  ...
  const cmd = useCommand((res) => {
    if (!res.success) {
      toast.error(
        `Tải lệnh thất bại: ${res?.error.message} (${res?.error.code})`
      );
      ...
      updateMutation.mutate(
        ...
        },
        {
          onSuccess(res) {
            if (res.success) {
              toast.success(
                "Lệnh đã lưu! Hãy triển khai lệnh để thay đổi có hiệu lực trên Discord."
              );
            } else {
              toast.error(
                `Cập nhật lệnh thất bại: ${res.error.message} (${res.error.code})`
              );
            }
          },
          onSettled() {
  ...
  return (
    <div className="flex min-h-[100dvh] w-full flex-col">
      <Head>
        <title>Quản lý lệnh | Vibe Bot</title>
      </Head>
      {cmd && (
        <FlowPage
