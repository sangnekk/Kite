...
function AppMessagePageInner() {
  ...
  const message = useMessage((res) => {
    if (!res.success) {
      toast.error(
        `Tải tin nhắn thất bại: ${res?.error.message} (${res?.error.code})`
      );
  ...
  useEffect(() => {
    ...
    try {
      ...
      toast.error(`Phân tích dữ liệu tin nhắn thất bại: ${e}`);
    }
  ...
  const save = useCallback(() => {
    ...
    updateMutation.mutate(
      ...
      },
      {
        onSuccess(res) {
          if (res.success) {
            toast.success("Đã lưu tin nhắn!");
          } else {
            toast.error(
              `Cập nhật tin nhắn thất bại: ${res.error.message} (${res.error.code})`
            );
          }
        },
        onSettled() {
  ...
  return (
    <div className="flex min-h-[100dvh] w-full flex-col">
      <Head>
        <title>Quản lý tin nhắn | Vibe Bot</title>
      </Head>
