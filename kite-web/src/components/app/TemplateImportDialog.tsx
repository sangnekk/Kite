  ...
  const handleImport = useCallback(() => {
    for (const input of template.inputs) {
      if (input.required && !inputValues[input.key]) {
        toast.error(`${input.label} là bắt buộc`);
        return;
      }
    }
      ...
      commandsImportMutation.mutate(
        { commands },
        {
          onSuccess: (res) => {
            if (res.success) {
              toast.success(
                `Đã nhập ${commands.length} lệnh thành công`
              );
            } else {
              toast.error(
                `Nhập lệnh thất bại: ${res.error.message} (${res.error.code})`
              );
            }
          },
        }
      ...
      eventListenersImportMutation.mutate(
        { event_listeners: eventListeners },
        {
          onSuccess: (res) => {
            if (res.success) {
              toast.success(
                `Đã nhập ${eventListeners.length} sự kiện thành công`
              );
            } else {
              toast.error(
                `Nhập sự kiện thất bại: ${res.error.message} (${res.error.code})`
              );
            }
          },
        }
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="overflow-y-auto max-h-[90dvh] max-w-2xl">
        <DialogHeader>
          <DialogTitle className="mb-1">Mẫu {template.name}</DialogTitle>
        ...
        <div className="mb-3 mt-2 space-y-5">
          ...
          {!!template.eventListeners.length && (
            <div>
              <div className="mb-3">
                <div className="font-medium mb-0.5">Bộ lắng nghe sự kiện</div>
                <div className="text-sm text-muted-foreground">
                  Chọn các bộ lắng nghe sự kiện bạn muốn nhập.
                </div>
              </div>
              <div className="flex flex-col gap-3">
