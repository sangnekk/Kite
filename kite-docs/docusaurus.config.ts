import { themes as prismThemes } from "prism-react-renderer";
import type { Config } from "@docusaurus/types";
import type * as Preset from "@docusaurus/preset-classic";
import "dotenv/config";

const config: Config = {
  title: "Vibe Bot",
  tagline: "Tạo bot Discord dễ dàng",
  favicon: "img/favicon.ico",

  // Set the production url of your site here
  url: "https://docs.kite.onl",
  // Set the /<baseUrl>/ pathname under which your site is served
  // For GitHub pages deployment, it is often '/<projectName>/'
  baseUrl: "/",

  // GitHub pages deployment config.
  // If you aren't using GitHub pages, you don't need these.
  organizationName: "merlinfuchs", // Usually your GitHub org/user name.
  projectName: "kite", // Usually your repo name.

  onBrokenLinks: "throw",
  onBrokenMarkdownLinks: "warn",

  // Even if you don't use internationalization, you can use this field to set
  // useful metadata like html lang. For example, if your site is Chinese, you
  // may want to replace "en" with "zh-Hans".
  i18n: {
    defaultLocale: "en",
    locales: ["en"],
  },

  customFields: {
    appBaseUrl: process.env.APP_BASE_URL || "http://localhost:3000",
  },

  presets: [
    [
      "classic",
      {
        docs: {
          sidebarPath: "./sidebars.ts",
          // Please change this to your repo.
          // Remove this to remove the "edit this page" links.
          routeBasePath: "/",
          editUrl: "https://github.com/sangnekk/Kite/tree/main/kite-docs/",
        },
        blog: {
          showReadingTime: true,
          // Please change this to your repo.
          // Remove this to remove the "edit this page" links.
          editUrl: "https://github.com/sangnekk/Kite/tree/main/kite-docs/",
        },
        theme: {
          customCss: "./src/css/custom.css",
        },
      } satisfies Preset.Options,
    ],
  ],

  themeConfig: {
    // Replace with your project's social card
    image: "img/logo.png",
    navbar: {
      title: "Vibe Bot",
      logo: {
        alt: "Vibe Bot Logo",
        src: "img/logo.svg",
      },
      items: [
        {
          type: "docSidebar",
          sidebarId: "tutorialSidebar",
          position: "left",
          label: "Tài liệu",
        },
        { to: "/blog", label: "Bài viết", position: "left" },
        {
          href: "https://github.com/sangnekk/Kite",
          label: "GitHub",
          position: "right",
        },
        {
          href: "https://discord.gg/rNd9jWHnXh",
          label: "Discord",
          position: "right",
        },
      ],
    },
    footer: {
      style: "dark",
      links: [
        {
          title: "Tài liệu",
          items: [
            {
              label: "Chào mừng",
              to: "/",
            },
            {
              label: "Bắt đầu",
              to: "/guides/getting-started",
            },
          ],
        },
        {
          title: "Cộng đồng",
          items: [
            {
              label: "Discord",
              href: "https://discord.gg/rNd9jWHnXh",
            },
            {
              label: "GitHub",
              href: "https://github.com/sangnekk/Kite",
            },
          ],
        },
        {
          title: "Pháp lý",
          items: [
            {
              label: "Điều khoản dịch vụ",
              href: "https://kite.onl/terms",
            },
            {
              label: "Chính sách bảo mật",
              href: "https://kite.onl/privacy",
            },
          ],
        },
      ],
      copyright: `Copyright © ${new Date().getFullYear()} tạo bởi Merlin Fuchs`,
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,

  plugins: [
    "./src/plugins/webpack-config.js",
    "./src/plugins/tailwind-config.js",
  ],
};

export default config;
